  ...
  useEffect(() => {
    ...
    const scrollStep = () => {
      if (!isPaused) {
        ...
        // Infinite loop: reset scroll when reaching start
    ...
    <section className="bg-gradient-to-b from-gray-50 via-white to-gray-50 py-10 rounded-lg relative">
      ...
      >
        {[...internships, ...internships].map((it, idx) => (
          ...
          >
            <div className="overflow-hidden rounded-lg shadow-md inline-block w-48 sm:w-56 md:w-64 h-40 sm:h-48 md:h-56">
              <img
                src={it.image}
                alt={it.title}
